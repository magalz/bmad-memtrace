...
4.5. **Gather structural context** — if Memtrace is available for this project:
   ...
   Use the Memtrace MCP tool `list_indexed_repositories` to check if the project root is indexed. Filter the results to match the actual project repo path — if multiple repos are indexed, select the one whose `repo_path` most closely matches the `{project-root}`. If an indexed repo matches:
