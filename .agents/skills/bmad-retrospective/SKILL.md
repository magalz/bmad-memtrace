...
- Document test coverage gaps
  ...
  <action>Check Memtrace availability for structural evolution analysis:
...
<check if="memtrace_available == true">
  <output>
Amelia (Developer): "While I have the story records loaded, let me also check what the codebase graph reveals about this sprint's structural impact..."

Charlie (Senior Dev): "The graph? You mean Memtrace can show us what actually changed structurally?"

Amelia (Developer): "Exactly. The story files tell us what developers REPORTED. The graph tells us what actually HAPPENED to the code."
  </output>

  <action>Graph-Based Structural Evolution Analysis:
  ...
  **E) Orphan Risk Assessment:**
  ...
  <action>Validate all queries completed successfully (no timeout errors)</action>
  <action>Process ALL queries STRICTLY SEQUENTIALLY using `for...of` with `await` — NEVER Promise.all</action>
  <action>Cap each query result at 200 symbols to stay within token budget</action>

  <action>Synthesize Structural Insights:
  ...
  <output>
Amelia (Developer): "The graph data adds some interesting dimensions to what the story files told us."

Charlie (Senior Dev): "Like what?"

Amelia (Developer): "{{summary_from_structural_insights}}"

{user_name} (Project Lead): [Can react to structural findings]

Amelia (Developer): "Let me now combine this with the story file patterns I extracted..."
  </output>
</check>

<check if="memtrace_available == false">
  <output>
Amelia (Developer): "I tried to check the codebase graph for structural data, but Memtrace isn't available right now."

Charlie (Senior Dev): "No worries — we still have all the story records and dev notes."

Amelia (Developer): "Right. Let me continue with the story file pattern analysis."
  </output>
  <action>Set `memtrace_available = false` for use in Step 8</action>
</check>
...
<check if="memtrace_available == true AND {{memtrace_structural_insights}}.available == true">
  <output>
Amelia (Developer): "Before we finalize action items, let me share what the structural analysis revealed. This is objective data from the codebase graph — it may confirm what we've been discussing, or flag things we missed."

Charlie (Senior Dev): "I'm curious how our gut feelings match up with the data."
  </output>

  <action>Present Structural Technical Debt Evidence:
  ...
  **Blast Radius Envelope:**
  ...
  <output>
Charlie (Senior Dev): [Reacts to data — may confirm or challenge] "{{reaction_to_data}}"

Dana (QA Engineer): "{{structural_data_driven_qa_insight}}"

Alice (Product Owner): "So the data shows {{data_driven_product_insight}}. That helps us prioritize."

Amelia (Developer): "{user_name}, the structural data suggests {{key_takeaway}}. How does this align with what you observed during the sprint?"
  </output>

  <action>WAIT for {user_name} to respond to structural findings</action>

  <action>Use structural data to RECOMMEND technical debt action items:
  ...
  <output>
Amelia (Developer): "Based on the structural data, I'd recommend we add these items to our action plan... but the decision is ours."

Alice (Product Owner): "The complexity hotspots concern me. {{priority_opinion}}"

Charlie (Senior Dev): "I can own the {{specific_item}} refactor. Estimated {{hours}} hours."
  </output>

  <action>Integrate structural data recommendations into the existing action item synthesis below</action>
</check>

<check if="memtrace_available == false OR {{memtrace_structural_insights}}.available == false">
  <output>
Amelia (Developer): "Without structural graph data this time, our action items will be based on the team discussion and story records — which is still solid."

Charlie (Senior Dev): "We've done good retros without the graph before. Let's keep going."
  </output>
  <action>Proceed with existing heuristic action item synthesis</action>
</check>
