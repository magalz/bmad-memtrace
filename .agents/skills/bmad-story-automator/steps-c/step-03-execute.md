...
**Propagate Memtrace state:**
- `memtrace_state` is stored in the state document frontmatter from step-01-init.md or step-01b-continue.md.
- When spawning Tmux sessions below, the state document path is passed via `--state-file`. Each spawned story workflow reads `memtrace_state` from its own state file to determine MCP availability.
- To re-check Memtrace health between epics, call `list_indexed_repositories` before the next epic's first story. The `build-cmd` tool passes `--state-file` to each session, so `memtrace_state` is available as a context variable in the spawned workflow.
